...
/*   Updated: 2022/03/07 18:16:33 by hnaciri-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_and_draw(int key_code, t_data *data)
{
	...
	if (data->move != j)
	{
		...
		s = ft_itoa (data->move);
		mlx_string_put (data->mlx, data->mlx_win, 20, 20, 100, s);
		free (s);
	}
	return (0);
}
...
void	ft_display(char *map, t_data *data)
{
	...
	ft_count_coins (data);
	ft_count_enemies (data);
	ft_count_player (data->the_map, data->array[1]);
	ft_draw (data);
	ft_enemies (data);
	mlx_key_hook (data->mlx_win, ft_check_and_draw, data);
	mlx_hook(data->mlx_win, 17, 0, ft_lose_red, data);
